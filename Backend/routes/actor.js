...
// get movie of actor
  router.get("/actor/mov/:actId", async function (req, res, next) {
    ...
    try {  
      const [rows, fields] = await pool.query("SELECT act_fname, act_lname, mov_id, mov_title, mov_rate, mov_pic FROM movie join movie_cast using(mov_id) join actor using(act_id) where act_id = ?",
      [req.params.actId]);
      // console.log(rows);
      return res.json(rows);
...
// get movie of director
  router.get("/dir/mov/:dirId", async function (req, res, next) {
    ...
    try {  
      const [rows, fields] = await pool.query("SELECT dir_fname, dir_lname, mov_id, mov_title, mov_rate, mov_pic FROM movie join movie_director using(mov_id) join director using(dir_id) where dir_id = ?",
      [req.params.dirId]);
      // console.log(rows);
      return res.json(rows);
